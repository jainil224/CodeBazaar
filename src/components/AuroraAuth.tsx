...
export default function AuroraAuth({ onClose }: AuroraAuthProps) {
  ...
  const handleSocialLogin = async (platform: string) => {
    if (platform === 'Google') {
      ...
      alert("GitHub authentication is not supported. Please use Google or Email/Password.");
    }
  };
            ...
            <>
              ...
              <div className="space-y-2 text-left">
                <h2 className="text-3xl font-medium tracking-tight text-white">
                  {isResetLinkSent ? 'Check Your Inbox' : 'Reset Password'}
                </h2>
                <p className="text-white/40 text-sm">
                  {isResetLinkSent 
                    ? 'We have sent a recovery email with a password reset link.' 
              ...
              {isResetLinkSent ? (
                <div className="space-y-6">
                  <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl text-xs text-emerald-400 text-center leading-relaxed">
                    If an account exists for this email, a password reset link has been sent to <span className="font-semibold text-white">{email}</span>.
                  </div>
                  <button
                    onClick={() => {
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <div className="text-center">
                    <button 
                      ...
                      className="text-xs font-semibold text-white/50 hover:text-white underline transition-colors cursor-pointer bg-transparent border-none"
                    >
                      Back to Log In
                    </button>
                  </div>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="space-y-6 lg:space-y-5">
                ...
                <div className="flex flex-col gap-2 w-full text-left">
                  ...
                  <p className="text-[11px] text-white/40 mt-1">Requires at least 6 symbols.</p>
                </div>
