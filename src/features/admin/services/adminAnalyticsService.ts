...
export async function fetchDashboardAnalytics(filter: DateRangeFilter): Promise<DashboardStats> {
  ...
  // 10. Compile orders lists
  // Map all current-period transactions for the Orders tab (no cap)
  const mapOrder = (tx: any) => ({
    id: tx.id,
  ...
  const allOrders = currentTxs.map(mapOrder);
  const recentOrders = allOrders.slice(0, 10); // dashboard widget shows 10 most recent
...
/**
 * Purges/deletes all development test documents from 'analytics_events'.
 */
export async function clearAllAnalyticsEvents(): Promise<void> {
  const eventsCollection = collection(db, 'analytics_events');
  const snapshot = await getDocs(eventsCollection);
  const deletePromises = snapshot.docs.map(docSnap => deleteDoc(doc(db, 'analytics_events', docSnap.id)));
